...
class KNearestNeighbor:
  ...
  def compute_distances_no_loops(self, X):
    ...
    #########################################################################
    X_Dot_X_train = X.dot(self.X_train.transpose())
    X = X*X
    X = np.sum(X, axis = 1)
    X_new = np.tile(X.reshape((num_test,1)),(1,num_train))
    X_train = self.X_train * self.X_train
    X_train = np.sum(X_train, axis = 1)
    X_train_new = np.tile(X_train,(num_test,1))
    dists = np.sqrt( X_new + X_train_new - 2*X_Dot_X_train)
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in xrange(num_test):
      ...
      #########################################################################
      dist_sort_arg = np.argsort(dists[i,:])
      closest_y = self.y_train[dist_sort_arg[0:k]]
      closest_y = np.array(closest_y, dtype = 'int')
      #########################################################################
      # TODO:                                                                 #
